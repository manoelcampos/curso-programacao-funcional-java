...
public class StudentGenerator {
     ...
     *             Passando um valor diferente a cada instância de questoes.StudentGenerator
     ...
     *
     * @param course Lista de cursos existentes
     ...
     * @return
     */
    private Curso randCourse(final Curso[] course, final boolean enableNull){
        if(enableNull){
            final int i = randInt(course.length+1);
            return i < course.length ? course[i] : null;
        }

        return course[randInt(course.length)];
    }
